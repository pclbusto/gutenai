...
<body><h1>Chapter 1</h1><p>Hello, EPUB!</p></body></html>"""
    ...
    def _kind_of(self, mi: ManifestItem) -> str:
        if "nav" in (mi.properties or "").split():
            return KIND_NAV
        if "cover-image" in (mi.properties or "").split():
            return KIND_COVER
        return MEDIA_TO_KIND.get(mi.media_type.lower(), "other")
    ...
    def rename_item(self, id_or_href: str, new_href: str) -> None:
        """Renombra el href en el manifest y mueve el archivo en disco.
        NO reescribe enlaces internos (XHTML/CSS/nav)."""
        assert self.opf_tree is not None
        mi = self._get_item(id_or_href)
        old_href = mi.href
        # mover archivo
        src = (self.opf_dir / old_href).resolve()
        dst = (self.opf_dir / new_href).resolve()
        ...
        self._save_opf()
        # Warning para que sepas que faltan enlaces internos
        # (podés enganchar un sistema de mensajes/eventos si querés)
...
if __name__ == "__main__":
    ...
    for item in core.list_items(KIND_STYLE):      # manifest completo
